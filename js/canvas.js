...
document.querySelectorAll('.crop-handle').forEach(ch => {
  ch.addEventListener('mousedown', e => {
    e.stopPropagation(); e.preventDefault();
    cropDrag = {
      active: true, handle: ch.dataset.ch,
...
function renderContextMenu() {
  if (!ctxMenu) {
    ...
    document.addEventListener('click', e => {
      if (ctxMenu.classList.contains('active')) {
...
function showContextMenu(e, type, idx, el) {
  e.preventDefault();
  e.stopPropagation();

  if (sel.type !== type || sel.idx !== idx) { selectEl(type, activeCardIdx, idx); }

  if (!ctxMenu) renderContextMenu();

  let html = '';
  if (type === 'text') { html += `<div class="context-item" onclick="enterEditMode(getElDOM('${type}', ${idx}))">✏️ Edit Text</div>`; }
  else if (type === 'img') { html += `<div class="context-item" onclick="startCrop()">✂️ Crop Image</div>`; }

  html += `
    <div class="context-divider"></div>
    <div class="context-item" onclick="moveElementLayer(1)">⬆️ Bring Forward</div>
    <div class="context-item" onclick="moveElementLayer(-1)">⬇️ Send Backward</div>
    <div class="context-divider"></div>
    <div class="context-item danger" onclick="deleteSelected()">🗑️ Delete Element</div>
  `;
...
function moveElementLayer(dir) {
  if (!sel.type || sel.type === 'header') return;
  const card = getCard();
  const arr = sel.type === 'text' ? card.texts : card.imgs;
  const idx = sel.idx;
  if (dir === 1 && idx < arr.length - 1) {
    const temp = arr[idx]; arr[idx] = arr[idx + 1]; arr[idx + 1] = temp;
    sel.idx = idx + 1;
    pushHist(); renderCanvas(); renderPropsPanel(); updateSelOverlay();
  } else if (dir === -1 && idx > 0) {
    const temp = arr[idx]; arr[idx] = arr[idx - 1]; arr[idx - 1] = temp;
    sel.idx = idx - 1;
    pushHist(); renderCanvas(); renderPropsPanel(); updateSelOverlay();
